...
class AlbumController: UIViewController, PKCanvasViewDelegate, PKToolPickerObserver, UIImagePickerControllerDelegate & UINavigationControllerDelegate,UIPopoverPresentationControllerDelegate, UIScreenshotServiceDelegate, AddListoryAlbumControllerDataSource {
    ...
    var collectionView = ListoryAlbumController()
...
//    var imageDataBase = DataBaseHelper()      <- DataBaseHelper
    ...
    func loadImages() {
        if let loadedImages = imageDataBase.retrieveAllImage(){
            images = loadedImages
        }
    }
...
//MARL: - Coredata Extension
extension AlbumController: AddListoryAlbumControllerDelegate {
